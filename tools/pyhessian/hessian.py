...
from tools.pyhessian.hutils import \
...
class Hessian():
    ...
    def trace_per_layer(self, maxIter=100, tol=1e-3):
        """
        compute the trace of hessian using Hutchinson's method
        maxIter: maximum iterations used to compute trace
        tol: the relative tolerance
        ...
        for i in range(maxIter):
            ...
            # : compute per layer
            trace_vhv.append([])
            for each_Hv, each_v in zip(Hv, v):
                trace_vhv[i].append(group_product(each_Hv, each_v).cpu().item())

            # : compute means... (for all layers)
            for j in range(len(self.params)):
                each_trace = [each_vhv[j] for each_vhv in trace_vhv]
                trace[j]   = np.mean(each_trace)

            # : return ...
            if self._check_convergence(trace_vhv, trace, tol):
                return self._mean_traces(trace_vhv)

        # : post process
        return self._mean_traces(trace_vhv)

    def _check_convergence(self, trace_vhv, trace, tol):
        output = True
        for i in range(len(self.params)):
            each_tmean  = [each_vhv[i] for each_vhv in trace_vhv]
            each_trace  = trace[i]
            each_output = abs((each_tmean - each_trace) / (each_trace + 1e-6)) < tol
            output = each_output and output
        return output

    def _mean_traces(self, trace_vhv):
        out_traces = [0.] * len(self.params)
        for i in range(len(self.params)):
            out_traces[i] = np.mean([each_vhv[i] for each_vhv in trace_vhv])
        return out_traces
